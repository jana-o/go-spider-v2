...
func match(matcher Matcher, l *link, baseURL string, ch chan<- *link) {
	sortResult, err := matcher.sort(l, baseURL)
	if err != nil {
		log.Println(err)
		return
	}
...
//display results blocks channel until result is written
func display(fr *fetchResult, ch chan *link) {
	//map would be better (as mentioned)
	...
	countType := 0
	for _, v := range links {
		if v.linkType == "internal" {
			countType++
		}
	}
	counter := 0
	for _, v := range links {
		if v.inaccessible == true {
			counter++
		}
	}
	fmt.Printf("Amount of external: %d, internal links: %d\nAmount of inaccessible links: %d\n", length-countType, countType, counter)
	fmt.Printf("Contains login link is: %t\n", haslogin)
}
